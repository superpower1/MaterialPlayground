...
import {latlong} from './geoLocation';
...
class Map extends Component {
  componentDidMount() {
    setTimeout(() => {
      const ammap = this.refs.ammap;
      const {chart} = ammap.state;
      chart.zoomToGroup('custom');
    }, 1000)
  }
  ...
  render() {
    ...
    const valueArr = mapData.map(dataItem => dataItem.value);
    ...
    const images = mapData.map(dataItem => {
      const {code, name, value, color} = dataItem;
    ...
    const config = {
      ...
      "dataProvider": {
        "map": "worldLow",
        "areas": [
          {"id": "AU", "groupId": "custom"},
          {"id": "GB", "groupId": "custom"},
        ],
        "images": images
      },
      "export": {
    ...
    return (
      <AmCharts.React ref="ammap" options={config} style={{width: '800px', height: '500px'}}/>
    );
  }
}
